...
class CreateGoodsAttributeTable extends Migration
{
    ...
    public function up()
    {
         Schema::create('goods_attribute', function (Blueprint $table) {
			...
			$table->tinyInteger('attr_index')->default(0)->unsigned()->comment('0不需要检索 1关键字检索 2范围检索');
			$table->tinyInteger('attr_type')->default(0)->unsigned()->comment('0唯一属性 1单选属性 2复选属性');
			$table->tinyInteger('attr_input_type')->unsigned()->default(0)->comment('0 手工录入 1从列表中选择 2多行文本框');
