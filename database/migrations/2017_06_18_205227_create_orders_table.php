...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->string('email',60)->nullable()->comment('邮件');
            ...
            $table->string('pay_name',32)->default("")->comment('支付方式名称');
            $table->string('invoice_title',256)->default("")->comment('发票抬头');
            $table->decimal('goods_price',10,2)->defaule(0.00)->unsigned()->comment('商品总价');
            $table->decimal('shipping_price',10,2)->defaule(0.00)->unsigned()->comment('邮费');
            $table->decimal('user_money',10,2)->defaule(0.00)->comment('使用余额');
            $table->decimal('coupon_price',10,2)->defaule(0.00)->comment('优惠券抵扣');
            $table->integer('integral')->defaule(0)->comment('使用积分');
            $table->decimal('integral_money',10,2)->defaule(0.00)->comment('使用积分抵多少钱');
            $table->decimal('order_amount',10,2)->defaule(0.00)->unsigned()->comment('应付款金额');
            $table->decimal('total_amount',10,2)->defaule(0.00)->unsigned()->comment('订单总价');
            ...
            $table->tinyInteger('order_prom_type')->defaule(0)->comment('0默认1抢购2团购3优惠4预售');
            $table->unsignedInteger('order_prom_id')->defaule(0)->comment('活动id');
            $table->decimal('order_prom_amount',10,2)->defaule(0.00)->comment('活动优惠金额');
            $table->decimal('discount',10,2)->defaule(0.00)->comment('价格调整');
            $table->string('user_note',255)->defaule("")->comment('用户备注');
            $table->string('admin_note',255)->defaule("")->comment('管理员备注');
            $table->string('parent_sn')->nullable()->comment('父单单号');
            $table->timestamps();
