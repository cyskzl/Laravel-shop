...
class CreateOrdersDetailsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('orders_details', function (Blueprint $table) {
            ...
            $table->json('goods_info')->comment('商品名称');
            $table->string('goods_sn')->index()->comment('商品货号');
            $table->smallInteger('goods_num')->unsigned()->comment('购买数量');
            $table->decimal('goods_price',10,2)->defaule(0.00)->unsigned()->comment('商品价格');
            $table->decimal('cost_price',10,2)->defaule(0.00)->unsigned()->comment('商品成本价');
            $table->decimal('member_goods_price',10,2)->defaule(0.00)->unsigned()->comment('会员折扣价');
            $table->mediumInteger('give_integral')->defaule(0)->unsigned()->comment('购买商品赠送积分');
            ...
            $table->unsignedInteger('prom_id')->nullable()->index()->comment('活动id');
