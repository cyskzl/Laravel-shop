...
@section('x-body')
    <form class="layui-form layui-form-pane">
        <input type="hidden" value="{{$trendpromotion->id}}" name="id">
        <div class="layui-form-item">
            <label class="layui-form-label" style="width: 100px">名称</label>
            <div class="layui-input-block">
                <input type="text" value="{{$trendpromotion->name}}" name="name" required="" lay-verify="required" placeholder="请输入名称" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-inline">
                ...
                <div class="layui-input-block">
                    <select lay-verify="required" name="top_cate_id">
                        ...
                        @foreach($cates as $v)
                            <option @if($trendpromotion->top_cate_id == $v->id) selected @endif value="{{$v->id}}">{{$v->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="sort" value="{{$trendpromotion->sort}}" required="" lay-verify="required" placeholder="请输入排序" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">描述</label>
            <div class="layui-input-block">
                <textarea placeholder="请输入描述" class="layui-textarea" name="desc">{{$trendpromotion->desc}}</textarea>
            </div>
        </div>
        <div class="layui-form-item">
    ...
    <script>
        ...
        layui.use(['form','layer','layedit'], function(){
            ...
            form.on('submit(add)', function(data){
                ...
                $.ajax({
                    type: 'post',
                    url:  '/admin/trendPromotion/'+data.field.id,
                    dataType: 'json',
