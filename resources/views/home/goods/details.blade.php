		...
		<div class="recomme fl">
			...
			<ul>
				@foreach($recommend as $commend)
					@if($commend->goods_id != $goods_id)
						<li>
							<a href="{{url('home/goodsDetail/'.$commend->goods_id)}}">
								<!--产品样式 -->
								<span class="cp-img"><img src="{{rtrim($commend->original_img, ',')}}" alt=""/></span>
								<!-- 产品牌子-->
								<span class="cp-brand">{{getBrand($commend->brand_id)}}</span>
								<!-- 产品名字-->
								<span class="cp-name">{{$commend->goods_name}}</span>
								<!-- 产品价格-->
								<span class="cp-price">￥{{$commend->shop_price}}</span>
							</a>
						</li>
					@endif
				@endforeach
