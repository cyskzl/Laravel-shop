 ...
 */
// $(function() {
    function oneUpload(uploadPath,token ){
        $('#file_oneupload').uploadify({
            'uploadLimit': 1,                               //上传文件设置
            ...
            if (data) {
                ...
                //将目录的路径加到img中
                $("#uploadone").append($(str));
                //查找动态生成的img标签
                var imgsrc = $('#uploadone').find('img');
                //生成第一张图片时候，走判断
                if (imgsrc.attr('src') !== null) {
                    ...
                    for (var i = 0; i < imgsrc.length; i++) {
                        ...
                        // for (var j = 0; j < array.length; j++) {
                            ...
                            switch (array.length) {
                                case 1:
                                        $('#imgone').attr('value', array[0]);
                                    break;
                                case 2:
                                        $('#imgone').attr('value', array[0] + array[1]);
                                    break;
                                case 3:
                                        $('#imgone').attr('value', array[0] + array[1] + array[2]);
                                    break;
                                case 4:
                                        $('#imgone').attr('value', array[0] + array[1] + array[2] + array[3]);
                                    break;
                                case 5:
                                        $('#imgone').attr('value', array[0] + array[1] + array[2] + array[3] + array[4]);
                                    break;
                                default:
                                        $('#imgone').attr('value','');
                                    break;
 ...
 * 注：一定要与上传文件路由同名
 */
function delOneImg(delUrl){
    $( '#uploadone' ).on( 'click','#delimg', function(){
        var that = $(this);
        ...
        $.ajax({
            ...
            success  :   function (data) {
                var fileattr = $('#file_oneupload').data('uploadify');//获取上传文件的属性
                //获取自己设置的上传文件大小
                console.log(fileattr.settings);
                var uploadLimit=fileattr.settings.uploadLimit;
                //重置增加上传文件数,队列-1
                $('#file_oneupload').uploadify('settings','uploadLimit', ++uploadLimit);
